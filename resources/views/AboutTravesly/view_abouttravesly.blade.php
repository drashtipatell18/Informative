    ...
    <div class="container mt-5">
        <div class="card">
            <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                <h6 class="mb-0">About Travels List</h6>
                <a href="{{ route('travesly.create') }}" class="btn btn-primary custom-btn">Add About Travel</a>
            </div>
                                    ...
                                    <td class="text-center">
                                        ...
                                        @if ($firstImage)
                                            <img class="avatar-img rounded-circle"
                                                src="{{ asset('images/AboutTravesly/' . $firstImage) }}" alt="Product Image"
                                                style="width: 30px; height: 30px; object-fit: cover; border: 2px solid #dee2e6;"
                                                onerror="this.src='{{ asset('images/AboutTravesly/dummy_product.png') }}'">
                                        @else
                                            <img src="{{ asset('images/AboutTravesly/dummy_product.png') }}"
                                                alt="Default Product Image" class="img-fluid rounded-circle"
                                                style="width: 30px; height: 30px; object-fit: cover; border: 2px solid #dee2e6;">
                                        @endif
                                    </td>
                                    <td class="align-middle text-center text-sm">
                                        <a href="{{ route('travesly.edit', $item->id) }}" class="btn btn-primary custom-btn"
                                            title="Edit"><i class="bi bi-pencil-square"></i></a>
                                        <form action="{{ route('travesly.destroy', $item->id) }}" method="POST"
